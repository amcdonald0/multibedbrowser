...
def show_projects():
    ...
    for p in projects_raw:
        ...
        for s in studies_raw:
            ...
            cur.execute("SELECT file_name, description FROM Files WHERE study_id = %s", (study_id,))
            files = [{'file_name': f[0], 'description': f[1]} for f in cur.fetchall()]
            studies.append({'study_name': study_name, 'description': study_desc, 'files': files})
        projects.append({'project_name': project_name, 'studies': studies})
    conn.close()
    return render_template('studies.html', projects=projects)
